    ...
    async def make_quote_plan(
        ...
        *,
        ...
        if self.s.USE_ECONO:
            ...
            if self.s.USE_HURST:
                feats.hurst = float(hurst_rs(r[-400:])) if r.size >= 120 else 0.5
                if feats.hurst < self.s.HURST_MIN:
                    return None
            if self.s.USE_ENTROPY:
                feats.entropy = float(shannon_entropy_returns(r[-400:])) if r.size >= 120 else 9.9
                if feats.entropy > self.s.ENTROPY_MAX:
                    return None
            if self.s.USE_LLE:
                feats.lle = float(lyapunov_rosenstein(series[-400:])) if len(series) >= 250 else 0.0
                if feats.lle > self.s.LLE_MAX:
                    return None
            if self.s.USE_MFDFA_PROXY:
