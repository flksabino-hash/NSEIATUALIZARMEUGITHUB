...
def hurst_rs(series: Sequence[float], max_lag: int = 20) -> float:
    """
    Rescaled range (R/S) based Hurst estimator (lightweight).
    >0.5 persistence; <0.5 anti-persistence.
    """
    x = np.asarray(series, dtype=float)
    if x.size < max(64, max_lag + 2):
        return 0.5
    ...
    hurst = poly[0] * 2.0
    return float(clamp(hurst, 0.0, 1.0))
...
def lyapunov_rosenstein(series: Sequence[float], emb_dim: int = 6, tau: int = 1, max_t: int = 20) -> float:
    """
